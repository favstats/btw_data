...
strukturdaten17 <- read_delim("data/btw17_strukturdaten.csv", 
  ...
  mutate(foreigners = str_replace(bev_u_663c_u_3e36_lkerung_am_31_12_2015_auslander_percent, ",", ".") %>% as.numeric) %>% 
  filter(!(str_detect(wahlkreis_name, "insgesamt|Insgesamt"))) %>% 
  select(wahlkreis_nr, unemployment, wanderung, sgbii, foreigners, wahlkreis_name, land) %>%
  mutate(wahlkreis_nr = as.integer(wahlkreis_nr))
...
```{r}
btw17_data <- btw17_data %>% 
  mutate(east_west = case_when(
    str_detect(land, "Berlin|Brand|Nieder|Sachs") ~ "East Germany",
    T ~ "West Germany"
  ))

mod17 <- lm(perc_afd ~ foreigners, data = btw17_data)
r2_17 <- get_r2_label(mod17)


b_text <- broom::tidy(mod17) %>% 
  filter(term == "foreigners") %>% 
  mutate(text = paste0("b = ", round(estimate, 2), 
         ", ", get_labs(p.value),
         ", R Squared = ", r2_17)) %>% 
  .$text
...
gg_afd_foreigner17 <- btw17_data %>% 
  # filter(year == "BTW 2017") %>% 
  ggplot(aes(foreigners, perc_afd)) +
  geom_smooth(se = F, alpha = .2, color = "black", size = 0.3) +
  geom_point(aes(color = east_west, shape = east_west), alpha = 0.8) +
  geom_smooth(method = "lm") +
  theme_minimal() +
  geom_text(aes(y = 34, x = 25, label = b_text), show.legend = F) +
  labs(x = "Share of Foreigners in %", y = "Vote Share AfD in %",
       title = "AfD Vote Share and Share of Foreigners in the 2017 German Election",
       caption = "N = 299 Electoral Districts     \nData from govdata.de and bundeswahlleiter.de   \n@favstats; favstats.eu    ")   +
  theme(title = element_text(face = "bold"), 
        strip.background = element_rect(fill = "lightgrey"), 
        strip.text = element_text(face = "bold"),
    # plot.margin=unit(c(23,0,15,0),"mm"),
        legend.position = "bottom") +
  ggthemes::scale_color_colorblind("Region") +
  scale_shape("Region")
...
tidytemplate::ggsave_it(gg_afd_foreigner17, width = 10, height = 6)
```
...
mod_east <- lm(perc_npd ~ foreigners, data = btw17_data %>% filter(east_west == "East Germany"))
mod_west <- lm(perc_npd ~ foreigners, data = btw17_data %>% filter(east_west == "West Germany"))

r2_east <- get_r2_label(mod_east)
r2_west <- get_r2_label(mod_west)

label_dat <- tibble(east_west = c("East Germany", "West Germany"),
     text =   c(
broom::tidy(mod_east) %>% 
  filter(term == "foreigners") %>% 
  mutate(text = paste0("b = ", round(estimate, 2), 
...
gg_afd_foreigner17_eastwest <- btw17_data %>% 
  ggplot(aes(foreigners, perc_afd, color = east_west)) +
  geom_smooth(se = F, alpha = .2, color = "red", size = 0.5) +
  geom_point(aes(shape = east_west), alpha = 0.8) +
  geom_smooth(method = "lm") +
  theme_minimal() +
  # geom_text(data = label_dat, aes(y = 34, x = 15, label = text), show.legend = F) +
  labs(x = "Share of Foreigners in %", y = "Vote Share AfD in %",
       title = "AfD Vote Share and Share of Foreigners in the 2017 German Elections",
       caption = "N = 299 Electoral Districts\nData from govdata.de and bundeswahlleiter.de\n@FabioFavusMaxim; favstats.eu")   +
  theme(title = element_text(face = "bold"), 
        strip.background = element_rect(fill = "lightgrey"), 
        strip.text = element_text(face = "bold"),
...
gg_afd_foreigner17_eastwest

tidytemplate::ggsave_it(gg_afd_foreigner17_eastwest, width = 10, height = 6)
```
...
tidytemplate::save_it(btw_map)


btw_map <- tidytemplate::load_it("data/btw_map.Rdata")
...
btw_map17_foreign <- btw_map17 %>% 
  select(perc_afd, foreigners, long, lat, group, id) %>% 
  gather(key, value, -long, -lat, -group, -id) %>%
  mutate(key = ifelse(key == "perc_afd", "Vote Share AfD\n", "Share of Foreigners\n")) %>% 
  ...
  guides(fill = guide_colorbar(barwidth = 10, barheight = 0.5,
                title.position = "top", title.hjust = 0.5,
                label.theme = element_text(colour = "black", size = 11, angle = 0))) +
  labs(title = "German General Election 2017\nShare of Foreigners and AfD Results\n",
      caption = "299 Electoral Districts   \nData from Federal Returning Officer ('Bundeswahlleiter')   \n@favstats; favstats.eu   ")

btw_map17_foreign

tidytemplate::ggsave_it(btw_map17_foreign, width = 8, height = 8)
```


```{r,eval = F}
btw_map_combined <- cowplot::plot_grid(btw_map17_foreign, gg_afd_foreigner17, ncol = 2)

btw_map_combined

tidytemplate::ggsave_it(btw_map_combined, width = 15, height = 8)
```
